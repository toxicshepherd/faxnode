...
ExecStart=$INSTALL_DIR/venv/bin/gunicorn -k gevent -w 1 --worker-connections 1000 -b 0.0.0.0:$PORT --certfile $INSTALL_DIR/certs/server.crt --keyfile $INSTALL_DIR/certs/server.key --timeout 120 wsgi:app
Restart=on-failure
RestartSec=10
